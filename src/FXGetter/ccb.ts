...
const getCCBFXRates = async (): Promise<FXRate[]> => {
    const req = await axios.get(
        'http://www.ccb.com/cn/home/news/jshckpj_new2.xml',
        {
            headers: {
                'User-Agent':
    ...
    const settlements =
        parsed?.ReferencePriceSettlements?.ReferencePriceSettlement ?? [];
    ...
    const inv = (v: number | undefined) =>
        v && Number.isFinite(v) && v !== 0 ? 1 / v : undefined;
    ...
    for (const data of list) {
        ...
        if (!mapped?.name) {
            console.warn(
                '[CCB] Unknown foreign currency code:',
                fromCode3,
                'row =',
                data,
            );
        ...
        function bankUpdated(
            lstPrDt: string | number,
            lstPrTm: string | number,
        ): Date {
